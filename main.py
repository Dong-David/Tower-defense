...
audio = AudioManager()
shoot_sound = pygame.mixer.Sound("res/shoot.wav")
...
audio.set_playlist(random.sample(background_playlist, len(background_playlist)), volume=0.07)
audio.play_next()
...
def StartNextRound():
    ...
    round_number += 1
    towers_to_place = round_number + 5
    soldier_to_spawn = round_number + 5
                ...
                if spawn_mode:
                    if event.button == 1:
                        if PickBlock() is not None and not placment_collided and soldier_to_spawn > 0:
                            x, y, z = PickBlock()
                            new_soldier = Soldier.Soldier("sol", "Idle", 0, (x, y + 1, z), 100, 50)
